#!/usr/bin/env python3
"""
Parses the 14 SOF IEO practice markdown files (../../sof-ieo-practise/*.md)
into per-unit JSON data files (../data/unit-XX.json) plus a manifest
(../data/manifest.json), and generates the 14 thin HTML shell pages
...
MD_DIR = SUBJECT_ROOT / "sof-ieo-practise"
...
<html lang="en">
<head>
{HEAD_COMMON}  <title>Unit {unit_info['num']} — {unit_info['title']} | SOF IEO Practice</title>
</head>
<body>
  <div class="topbar">
    <nav class="breadcrumb" aria-label="Breadcrumb">
      ...
      <a class="crumb-link" href="index.html">📚 English</a>
      <span class="crumb-sep">›</span>
  ...
  <footer class="site-footer">SOF IEO English Olympiad Practice &middot; Unit {unit_info['num']} of 14</footer>
...
def index_html(manifest):
    grammar = [u for u in manifest if u["num"] <= 7]
    language = [u for u in manifest if u["num"] >= 8]
    grammar_cards = "".join(unit_card_html(u) for u in grammar)
    language_cards = "".join(unit_card_html(u) for u in language)
    return f"""<!DOCTYPE html>
<html lang="en">
<head>
{HEAD_COMMON}  <title>SOF IEO English Olympiad Practice</title>
</head>
<body>
  <div class="topbar">
    <nav class="breadcrumb" aria-label="Breadcrumb">
      <a class="crumb-link" href="../index.html">🏠 Home</a>
      <span class="crumb-sep">›</span>
      <span class="crumb-current">📚 English</span>
    </nav>
  </div>
  <div class="wrap">
    <div class="hero">
      <h1>📚 SOF IEO English Olympiad Practice</h1>
      <p>Tap a unit to start practicing — your progress is saved on this phone!</p>
    </div>

    <div class="group-heading">✏️ Grammar Basics</div>
    <div class="grid">
{grammar_cards}    </div>

    <div class="group-heading">📖 Language &amp; Reading Skills</div>
    <div class="grid">
{language_cards}    </div>
  </div>
  <footer class="site-footer">Made with care for a Class 2 English learner 🌟</footer>
