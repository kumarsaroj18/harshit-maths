#!/usr/bin/env python3
"""
Parses the 5 SOF NSO practice markdown files (../../sof-nso-practise/*.md)
into per-unit JSON data files (../data/unit-XX.json) plus a manifest
(../data/manifest.json), and generates the 5 thin HTML shell pages
...
MD_DIR = SUBJECT_ROOT / "sof-nso-practise"
...
def parse_unit(md_path):
    ...
    section_names = re.findall(r"^## Section [ABC] — (.+?) \(Q", text, re.MULTILINE)
    if len(section_names) != 3:
        raise ValueError(
            f"{md_path.name}: expected 3 section headings, found {len(section_names)}"
        )

    sections = []
    total_questions = 0
    for name, raw in zip(section_names, [section_a_raw, section_b_raw, section_c_raw]):
    ...
    if total_questions != 30:
        raise ValueError(
            f"{md_path.name}: expected 30 questions total, found {total_questions}"
        )
...
<html lang="en">
<head>
{HEAD_COMMON}  <title>Unit {unit_info['num']} — {unit_info['title']} | NSO Science Practice</title>
</head>
<body>
  <div class="topbar">
    <nav class="breadcrumb" aria-label="Breadcrumb">
      ...
      <a class="crumb-link" href="index.html">🔬 Science</a>
      <span class="crumb-sep">›</span>
  ...
  <footer class="site-footer">NSO Science Olympiad Practice &middot; Unit {unit_info['num']} of 5</footer>
...
def index_html(manifest):
    life_science = [u for u in manifest if u["num"] <= 4]
    earth_env = [u for u in manifest if u["num"] >= 5]
    life_science_cards = "".join(unit_card_html(u) for u in life_science)
    earth_env_cards = "".join(unit_card_html(u) for u in earth_env)
    return f"""<!DOCTYPE html>
<html lang="en">
<head>
{HEAD_COMMON}  <title>SOF NSO Science Olympiad Practice</title>
</head>
<body>
  <div class="topbar">
    <nav class="breadcrumb" aria-label="Breadcrumb">
      <a class="crumb-link" href="../index.html">🏠 Home</a>
      <span class="crumb-sep">›</span>
      <span class="crumb-current">🔬 Science</span>
    </nav>
  </div>
  <div class="wrap">
    <div class="hero">
      <h1>🔬 SOF NSO Science Olympiad Practice</h1>
      <p>Tap a unit to start practicing — your progress is saved on this phone!</p>
    </div>

    <div class="group-heading">🌿 Life Science</div>
    <div class="grid">
{life_science_cards}    </div>

    <div class="group-heading">🌍 Earth &amp; Environment</div>
    <div class="grid">
{earth_env_cards}    </div>
  </div>
  <footer class="site-footer">Made with care for a Class 2 scientist 🌟</footer>
