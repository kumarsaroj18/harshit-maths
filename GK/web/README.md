# SOF IGKO Practice — Web App

A mobile-friendly, interactive quiz version of the 11 worksheets in
`../sof-igko-practise/`. Tap an answer, get instant right/wrong
...
`../sof-igko-practise/`. After editing one, regenerate the JSON data
and HTML pages:
...
This re-parses all 11 files and rewrites `web/data/*.json` and every
`web/unit-*.html` + `web/index.html` (all inside `GK/web/`). It will
raise an error if a file doesn't have exactly 30 questions total or a
question's answer key entry is missing, so a malformed edit fails
loudly instead of silently breaking a page.
...
This folder is served at the `/gk/` path as part of the combined
