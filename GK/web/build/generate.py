#!/usr/bin/env python3
"""
Parses the 11 SOF IGKO practice markdown files (../../sof-igko-practise/*.md)
into per-unit JSON data files (../data/unit-XX.json) plus a manifest
(../data/manifest.json), and generates the 11 thin HTML shell pages
...
MD_DIR = SUBJECT_ROOT / "sof-igko-practise"
...
<html lang="en">
<head>
{HEAD_COMMON}  <title>Unit {unit_info['num']} — {unit_info['title']} | IGKO GK Practice</title>
</head>
<body>
  <div class="topbar">
    <nav class="breadcrumb" aria-label="Breadcrumb">
      ...
      <a class="crumb-link" href="index.html">🌟 GK</a>
      <span class="crumb-sep">›</span>
  ...
  <footer class="site-footer">IGKO GK Olympiad Practice &middot; Unit {unit_info['num']} of 11</footer>
...
def index_html(manifest):
    general = [u for u in manifest if u["num"] <= 9]
    current = [u for u in manifest if u["num"] >= 10]
    general_cards = "".join(unit_card_html(u) for u in general)
    current_cards = "".join(unit_card_html(u) for u in current)
    return f"""<!DOCTYPE html>
<html lang="en">
<head>
{HEAD_COMMON}  <title>SOF IGKO GK Olympiad Practice</title>
</head>
<body>
  <div class="topbar">
    <nav class="breadcrumb" aria-label="Breadcrumb">
      <a class="crumb-link" href="../index.html">🏠 Home</a>
      <span class="crumb-sep">›</span>
      <span class="crumb-current">🌟 GK</span>
    </nav>
  </div>
  <div class="wrap">
    <div class="hero">
      <h1>🌟 SOF IGKO GK Olympiad Practice</h1>
      <p>Tap a unit to start practicing — your progress is saved on this phone!</p>
    </div>

    <div class="group-heading">📚 General Awareness</div>
    <div class="grid">
{general_cards}    </div>

    <div class="group-heading">📰 Current Affairs &amp; Life Skills</div>
    <div class="grid">
{current_cards}    </div>
  </div>
  <footer class="site-footer">Made with care for a Class 2 GK champion 🌟</footer>
