#!/usr/bin/env python3
"""
Parses the 7 SOF ICSO practice markdown files (../../sof-icso-practise/*.md)
into per-unit JSON data files (../data/unit-XX.json) plus a manifest
(../data/manifest.json), and generates the 7 thin HTML shell pages
...
MD_DIR = SUBJECT_ROOT / "sof-icso-practise"
...
<html lang="en">
<head>
{HEAD_COMMON}  <title>Unit {unit_info['num']} — {unit_info['title']} | SOF ICSO Practice</title>
</head>
<body>
  <div class="topbar">
    <nav class="breadcrumb" aria-label="Breadcrumb">
      ...
      <a class="crumb-link" href="index.html">💻 ICSO</a>
      <span class="crumb-sep">›</span>
  ...
  <footer class="site-footer">SOF ICSO Olympiad Practice &middot; Unit {unit_info['num']} of 7</footer>
...
def index_html(manifest):
    reasoning = [u for u in manifest if u["num"] <= 1]
    computer = [u for u in manifest if u["num"] >= 2]
    reasoning_cards = "".join(unit_card_html(u) for u in reasoning)
    computer_cards = "".join(unit_card_html(u) for u in computer)
    return f"""<!DOCTYPE html>
<html lang="en">
<head>
{HEAD_COMMON}  <title>SOF ICSO Olympiad Practice</title>
</head>
<body>
  <div class="topbar">
    <nav class="breadcrumb" aria-label="Breadcrumb">
      <a class="crumb-link" href="../index.html">🏠 Home</a>
      <span class="crumb-sep">›</span>
      <span class="crumb-current">💻 ICSO</span>
    </nav>
  </div>
  <div class="wrap">
    <div class="hero">
      <h1>💻 SOF ICSO Olympiad Practice</h1>
      <p>Tap a unit to start practicing — your progress is saved on this phone!</p>
    </div>

    <div class="group-heading">🧠 Logical Reasoning</div>
    <div class="grid">
{reasoning_cards}    </div>

    <div class="group-heading">💻 Computer &amp; IT Skills</div>
    <div class="grid">
{computer_cards}    </div>
  </div>
  <footer class="site-footer">Made with care for a Class 2 cyber olympian 🌟</footer>
