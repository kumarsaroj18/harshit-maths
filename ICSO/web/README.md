# SOF ICSO Practice — Web App

A mobile-friendly, interactive quiz version of the 7 worksheets in
`../sof-icso-practise/`. Tap an answer, get instant right/wrong
...
`../sof-icso-practise/`. After editing one, regenerate the JSON data
and HTML pages:
...
This re-parses all 7 files and rewrites `web/data/*.json` and every
`web/unit-*.html` + `web/index.html` (all inside `ICSO/web/`). It
...
This folder is served at the `/icso/` path as part of the combined
